...
class Profile(models.Model):
    ...
    account = models.OneToOneField(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, verbose_name="アカウント情報")

    # よろずIDはprofileのIDでもあり、profile詳細ページのURLになる
    yorozu_id = models.CharField("YOROZU ID", max_length=20, default="", primary_key=True)
    ...
    twitter_account = models.CharField("twitterのアカウント", max_length=80, default="", blank=True)
    instagram_account = models.CharField("instagramのアカウント", max_length=80, default="", blank=True)
    facebook_account = models.CharField("facebookのアカウント", max_length=80, default="", blank=True)
